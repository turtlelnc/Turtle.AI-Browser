...
- **阅读模式**（工具栏按钮 / F9 / `tib.toggleReaderMode()`）：在页面里按"段落文本量 − 链接密度"
  给候选容器打分提取正文，剔除导航 / 侧栏 / 广告 / 评论 / 页脚 / 脚本，渲染成沉浸式阅读视图
  （Esc 或再按一次退出，退出时恢复页面滚动状态）。提取结果由页面回传（标题 / 字符数 / 段数）写进日志，
  验收断言"提取到的是正文而不是整页"（探针页：标题取自正文 h1、865 字符、6 段）。
  诚实标注：启发式提取，不等同于 Chrome/Edge 的阅读视图。
- **新增验收用例 7/8/9/10/11**：右键菜单自检、下载落盘与安全判定、不安全安装包只警告不拦截、
  命令行开关真的生效、阅读模式。用例 4 增加指纹回读断言。整套验收现在 11 个用例 61 项断言。
- **新增排查开关**：`--tib-menu-probe[=show]`（右键菜单自检，`show` 会真实弹出）、
  `--tib-reader-probe`（文章探针 + 进入/退出阅读模式）、
