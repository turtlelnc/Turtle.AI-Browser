...
// ---------------------------------------------------------------- 用例 11：阅读模式
console.log('\n用例 11：阅读模式（正文提取要真的做取舍，退出要干净）')
if (want(11)) {
  const r = await runCase('reader', ['--tib-reader-probe'], 30)
  const m = /阅读模式：已进入（标题=([^，]*)，正文 (\d+) 字符、(\d+) 段）/.exec(r.log)
  check('首个标签页换成了文章探针', /阅读模式自检：首个标签页改为文章探针/.test(r.log))
  check('成功进入阅读模式并回传正文信息', !!m, m ? '' : '未找到回传报告')
  check('提取到的是文章正文（标题取自正文标题）',
    !!m && m[1].includes('Chromium 嵌入方案'),
    m ? `标题=${m[1]}` : '')
  check('正文长度合理（>500 字符）', !!m && Number(m[2]) > 500, m ? `${m[2]} 字符` : '')
  check('段落数合理（>=4 段）', !!m && Number(m[3]) >= 4, m ? `${m[3]} 段` : '')
  check('退出后页面恢复原样', /阅读模式：已退出，页面恢复原样/.test(r.log))
  check('阅读模式全程进程稳定', r.survived >= 25, `实际 ${r.survived}s`)
}
