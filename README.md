...
**阅读模式**（工具栏按钮或 F9）：在页面里按"段落文本量 − 链接密度"给候选容器打分提取正文，
剔除导航 / 侧栏 / 广告 / 评论 / 页脚 / 脚本，渲染成 Apple 风格的沉浸式阅读视图（Esc 或再按一次退出）。
提取结果由页面回传（标题 / 字符数 / 段数）写进日志，验收脚本会断言"提取的是正文而不是整页"。
...
- **阅读模式是启发式提取**：按段落文本量与链接密度打分选正文容器，能应付常见文章页，
  但不等同于 Chrome/Edge 的阅读视图（没有分页、朗读、字号记忆，也不能保证每个站点都提取得干净）。
